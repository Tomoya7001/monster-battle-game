...
戦略性を高めるため、バトル結果に影響する運要素を完全排除。

| 排除項目 | 変更内容 |
|----------|----------|
| 命中率 | **100%固定**（外れる技は存在しない） |
| 回避率 | **廃止**→「次被ダメ半減」に変更 |
| クリ率％ | **廃止**→条件付きクリ確定に変更 |
| 命中↓ | **廃止**→防御↓または魔防↓に変更 |
| 状態異常付与率 | **100%確定**（威力でバランス） |

**例:**
- 「クリ率30%」→「相手HP50%以上でクリ確定」
- 「次回避」→「次被ダメ半減（1回）」
- 「命中↓」→「防御↓」または「魔防↓」
...
| damage_type | string | ダメージ種別（attack/magic/adaptive/fixed/none） |
...
### 10.5 クリティカル準備系（v12.2改修）

| 効果ID | 表現（ユーザー向け） | 計算式 |
|--------|---------------------|--------|
| `crit_prep_next` | 次の攻撃がクリティカル確定 | 次の攻撃時クリティカル = 100% |
| `crit_prep_3t` | 3T間クリティカル確定 | 3T間攻撃時クリティカル = 100% |
| `crit_on_weakness` | 弱点攻撃時クリティカル確定 | 弱点属性で攻撃時クリティカル = 100% |
| `crit_on_low_hp` | HP30%以下でクリティカル確定 | 自分HP ≤ 30%時クリティカル = 100% |
| `crit_multi_hit_2nd` | 2撃目がクリティカル確定 | 2回攻撃の2発目クリティカル = 100% |
...
| 効果ID | 表現（ユーザー向け） | 計算式 |
|--------|---------------------|--------|
| `fixed_dmg_20` | 20固定ダメージ | 相手HP -= 20（防御無視） |
| `fixed_dmg_30` | 30固定ダメージ | 相手HP -= 30（防御無視） |
...
### 10.16 現在HP依存ダメージ（v12.1追加）

| 効果ID | 表現（ユーザー向け） | 計算式 |
|--------|---------------------|--------|
| `current_hp_percent_50` | 相手の現在HPの半分ダメージ | 相手HP -= 相手現在HP × 50% |

### 10.17 コスト操作系（v12.1追加）

| 効果ID | 表現（ユーザー向け） | 計算式 |
|--------|---------------------|--------|
| `next_skill_cost_zero` | 次の同技のコスト0 | 次回同じ技を使用時、コスト消費 = 0 |
...
| バージョン | 日付 | 変更内容 |
|-----------|------|----------|
| **v12.4.0** | **2025-12-30** | **JSONマスターデータ同期、Support型奥義一覧修正（ルリア・レオン）、Attack型14体に修正、ラガ削除、リリス奥義名修正** |
| v3.4 | 2025-12-29 | クリティカルシステムv12.2改修（確率廃止→条件確定）、命中・回避系削除（運要素排除）、「急所」→「クリティカル」統一 |
| v3.3 | 2025-12-29 | **damage_type用語attack/magicに統一、fixed追加、Attack型奥15体に修正、新効果ID追加（current_hp_percent_50, next_skill_cost_zero, fixed_dmg_20/30）** |
